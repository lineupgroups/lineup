        ...
        <div className={`bg-white rounded-xl shadow-sm border border-gray-200 p-6 ${className}`}>
            ...
            <div className="space-y-4">
                {insights.map((insight, index) => (
                    <div key={index} className={`p-4 rounded-lg border ${insight.color} flex gap-4`}>
            ...
            <div className="mt-6 pt-4 border-t border-gray-100">
                <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-500">Based on {donations.length} donations</span>
                    <button className="text-blue-600 font-medium hover:text-blue-700">View Full Report</button>
                </div>
            </div>
        </div>
    );
